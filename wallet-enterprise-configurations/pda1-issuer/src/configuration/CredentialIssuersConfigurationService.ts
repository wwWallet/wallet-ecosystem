...
const issuerSigner: CredentialSigner = {
	sign: async function (payload, headers, disclosureFrame) {
		...
		const expirationDate = (() => {
			if (payload.vc.credentialSubject?.decisionOnApplicableLegislation?.validityPeriod?.endingDate) {
				const expirationDate = new Date(payload.vc.credentialSubject.decisionOnApplicableLegislation.validityPeriod.endingDate);
